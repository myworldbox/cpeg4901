...
int
    ...
    statSize = 22,
    optimizedProfitMode,
    bigValue = 100000000000,scanRange;
...
void createTrades()
{
    if (count < slotsToTrade)
    {
        ...
        if (direction == 1)
        {
            createTraining_2(biggestMACD, biggestMACD, biggestMACD);
        }
...
void createTraining_2(int x, int y, int z)
{
...
void Logic()
{
    ...
    createTrades();
    //FileClose(handle);
    GlobalVariableSet("opt_mode", opt_mode);
    GlobalVariableSet("mode", mode);
    ChartApplyTemplate(NULL,"indicatorsVersion.tpl");
/*
    double optimizedProfit, temp = opt_mode;
    for (int i = 0; i < statSize; i++)
    {
        opt_mode = i;
